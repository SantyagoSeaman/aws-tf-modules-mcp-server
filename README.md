...
```json
{
  "mcpServers": {
    "terraform-modules": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "ghcr.io/santyagoseaman/tfmodsearch:0.23.0"]
    }
  }
}
```
...
export TFMODSEARCH_IMAGE=ghcr.io/santyagoseaman/tfmodsearch:0.23.0
```
...
```bash
docker run --network none -i --rm ghcr.io/santyagoseaman/tfmodsearch:0.23.0 --warmup
```
...
docker run -d --name tfmodsearch-http --restart unless-stopped \
  ...
  ghcr.io/santyagoseaman/tfmodsearch:0.23.0 \
