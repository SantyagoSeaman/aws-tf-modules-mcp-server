...
- **A module doc leaked raw YAML front-matter into its search-result `description`** — `extract_description` skipped the `---` fences as horizontal rules but kept the block's key/value lines (`module_name:`, `keywords:`) as content (only `wafv2.md` opens with front-matter). `_strip_yaml_frontmatter` drops the leading block first.
...
- **The search index is not rebuilt and every embedding vector is byte-identical to 0.21.0** — one doc (s3-bucket) is re-encoded for the SSE-shape doc edit, but the edit lands past the embedding-model truncation window so its vector row is unchanged; search ranking and results are byte-identical to 0.21.0. `compute_scores` is a thin wrapper over the new `compute_scores_detailed`, so every existing caller (CLI, tests) is unaffected.
