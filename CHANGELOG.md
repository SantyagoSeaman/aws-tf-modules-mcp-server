...
Retrieval levers, revised (RC2) against a second measurement (Run 8) that showed the first cut of this release was net-negative: cost rose 9.7% and doc calls rose 22% while payload bytes fell only 4.4%. The corrected objective, measured across both runs' transcripts: one extra tool-call turn costs ~$0.093 blended, so optimizing bytes-per-call while adding calls is a guaranteed net loss. This revision removes the turn-adding `tie` verdict, strengthens the no-match signal so removing `tie` cannot silently mis-classify a catalog gap as a confident match, turns on the one measured turn-saving lever by default, and fixes two correctness defects surfaced by the same transcripts. A third measurement (Run 9) confirmed the forecast — A-fleet cost $1.76 below the 0.21.0 baseline, get_module calls down 94.6%, tie-driven reformulations gone — and surfaced three residual calibration-hygiene items, folded in here: the expand_top auto-inline gate is decoupled from (and stricter than) the confidence verdict so a catalog-gap false-high no longer drags a wrong module head into context, the single-snapshot version rule extends to the doc body's own version bullet, and non-top-1 result blurbs clip on a word boundary. The search index and the module docs corpus are untouched; search ranking is byte-identical to 0.21.0.

### Added

- **`search_modules` now returns a `confidence` verdict** — `"high"` (trust the top hit) or `"low"` (no confident catalog match) — plus a `hint` string on `"low"` (absent from JSON on `"high"`). The classifier uses two signals: whether the top hit earned a lexical component (exact module-name match or keyword overlap), and, for a lexical-but-not-exact top hit, whether its raw semantic similarity clears a floor (`SEARCH_SEM_FLOOR`, provisional at 0.88) — this catches an incidental-keyword catalog gap (an unrelated module lists the query term as a related keyword) that a lexical check alone cannot separate from a real match. `ScoredHit` gained `sem_sim`, the raw pre-min-max cosine similarity, comparable across queries (`compute_scores_detailed` in `tfmod_search_lib.py`, `_classify_confidence`/`SEARCH_SEM_FLOOR` in `tfmod_mcp_server.py`).
...
- **The expand_top auto-inline gate is decoupled from the confidence verdict and stricter (`_should_inline_top`)**: inlining a wrong 13–17K-char orientation head onto a catalog-gap false-high costs far more than the single get_module turn it saves on a real match, so the inline now fires only when the top hit is an exact-name match, or is a keyword match that both clears `SEARCH_SEM_FLOOR` and leads rank-2 by `SEARCH_INLINE_SCORE_MARGIN` (0.5, provisional; within-query score gap). The two-band verdict is unchanged — a marginal high-confidence hit still reports `"high"`, it just no longer auto-inlines. The debug log now emits `score`/`score_gap`/`inline` per search for A/B refinement. (Run 9: the sole source of the +11% payload delta on absence-proof workflows was a generous-band wrong-domain hit dragging an irrelevant doc into context.)
