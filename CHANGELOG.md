...
- New tests: `tests/integration/test_capability_coverage.py` (coverage-function unit tests: strong/weak/absent evidence, IDF weighting, the no-evidence fail-closed-on-contentless-query / fail-open-on-degraded-index split), `tests/integration/test_exact_match_boundary.py` (ranker hyphen-boundary unit tests), and the new `tests/integration/test_verdict_rule.py` (fresh, non-probe exemplars covering the new decision rule and its paraphrase-determinism guarantees).
...
- **Guidance pass on the server instructions and the plugin skills** (text only, no code/tool-schema change): the server instructions and the `aws-terraform-modules`/`tf-module`/`tf-grep`/`tf-migrate`/`tf-stack` skills now explain the new `confidence` verdict (a `"low"` is not proof a module is missing from the catalog, and an exclusion-phrased query can still grade `"high"` on the excluded technology), and the server instructions plus `aws-terraform-modules`/`tf-module` add explicit nested-leaf-shape discipline (check `examples` before writing any `any`/`object(...)` input; never leave a required nested block as a prose comment). Also dropped the stale hardcoded "54 modules" count from two skills in favor of count-free phrasing.
...
- A short, informal, or typo-laden query (e.g. "pls create s3 bucket", "help me set up redis", a misspelled module name) can still grade `"low"` on a correct, high-scoring top-1: an unknown/out-of-corpus token now contributes at the corpus's *mean* IDF weight rather than its max (calibrated down since the max weight measurably overweighed a single slang or typo token), but a query built mostly from such tokens can still fall short of the coverage threshold. A data-driven calibration study (165 golden queries plus a dedicated negative exemplar set) found no coverage threshold below the current one that rescues additional golden-correct queries without also admitting new wrong-domain false-highs, so the threshold is unchanged; this residual is disclosed rather than tuned away by chasing individual examples.

## [0.22.0] - 2026-07-16

[0.22.0]: https://github.com/SantyagoSeaman/tfmodsearch/releases/tag/v0.22.0

Retrieval levers, revised (RC2) against a second measurement (Run 8) that showed the first cut of this release was net-negative: cost rose 9.7% and doc calls rose 22% while payload bytes fell only 4.4%. The corrected objective, measured across both runs' transcripts: one extra tool-call turn costs ~$0.093 blended, so optimizing bytes-per-call while adding calls is a guaranteed net loss. This revision removes the turn-adding `tie` verdict, strengthens the no-match signal so removing `tie` cannot silently mis-classify a catalog gap as a confident match, turns on the one measured turn-saving lever by default, and fixes two correctness defects surfaced by the same transcripts. A third measurement (Run 9) confirmed the forecast — A-fleet cost $1.76 below the 0.21.0 baseline, get_module calls down 94.6%, tie-driven reformulations gone — and surfaced three residual calibration-hygiene items, folded in here: the single-snapshot version rule extends to the doc body's own version bullet, and non-top-1 result blurbs clip on a word boundary. A fourth measurement (Run 10) then confirmed those and reshaped the inline gate: the wrong-doc-inline failure is a capability mismatch, not a ranking-confidence one, so the verdict and the inline are now decided by ONE capability-aware signal (`_classify_confidence`) — a lexical-but-not-exact top hit is "high" (and therefore inlined) only when the query's central capability term actually appears in that module's doc, restoring the "high ⇒ doc inlined" contract and demoting a wrong-domain hit before it can drag its doc into context. Run 10 also identified consumer thinking tokens (not bytes or turns) as the dominant cost lever, which is why removing internal contradictions (version desync, wrong-domain inlines) pays out beyond its byte count. The search index is re-encoded drift-safe for two docs (s3-bucket byte-identical; wafv2's vector shifts only because a redundant front-matter block was removed), the golden retrieval set holds 172/172, and the ranker itself is unchanged from 0.21.0.

### Added

- **`search_modules` now returns a `confidence` verdict** — `"high"` (trust the top hit) or `"low"` (no confident catalog match) — plus a `hint` string on `"low"` (absent from JSON on `"high"`). The classifier is capability-aware: an exact module-name match is always `"high"`; a query with no lexical component at all is `"low"` (a semantic-only ceiling); a lexical-but-not-exact top hit is `"high"` only when BOTH the query's central capability term (its highest-BM25-IDF token, skipping generic-infra words) appears in that module's name/keywords/doc text (`_capability_covered`, the primary guard against a wrong-domain hit) AND its raw semantic similarity clears a floor (`SEARCH_SEM_FLOOR`, 0.88 — the secondary incidental-keyword guard). `ScoredHit` gained `sem_sim`, the raw pre-min-max cosine similarity, comparable across queries (`compute_scores_detailed` in `tfmod_search_lib.py`; `_classify_confidence`/`_capability_covered`/`SEARCH_SEM_FLOOR` in `tfmod_mcp_server.py`).
- **`grep_module_docs` matches carry an `enclosing` breadcrumb** — the nearest enclosing `- <name> | ...` row when a match lands on a continuation line of a multi-line input/output/resource row (a nested `object`/`map` type spans multiple physical lines), so the container name is never lost or back-filled incorrectly from surrounding prose.

### Changed

- **Measured net-negative: the `tie` verdict is removed.** It fired on 55% of searches, its "fetch both" hint was acted on 0/41 times with 0/41 benefit, and it provoked 28 extra search reformulations — the score ratio it was based on carried no signal separating a resolved near-tie from a catalog gap. The confidence domain is now `{"high", "low"}` only; an exact module-name match always classifies `"high"` regardless of semantic similarity.
- **`search_modules(expand_top=...)` now defaults to `True`** on a high-confidence search (previously opt-in, default `False`). A fleet-wide counterfactual (expand_top always on, tie removed) collapsed roughly 42 turns, more than covering the token cost of inlining the ~1.5K-char head; still never inlined on a `"low"` verdict, and `expand_top=False` still suppresses it for a pure-browse search.
- **The expand_top auto-inline is decided by the confidence verdict — one signal, not two.** `top_module_doc` is inlined iff the verdict is `"high"`, restoring the documented "high ⇒ doc inlined" contract. Run 10 showed the wrong-doc-inline failure is a *capability* mismatch (a wide-margin top hit whose doc never mentions the query's central term), which a score-gap gate is structurally blind to; the capability-overlap check in `_classify_confidence` now demotes such a hit to `"low"` before it can be inlined, while a thin-margin *correct* hit (capability-covered) inlines as it should. The RC-internal `_should_inline_top`/`SEARCH_INLINE_SCORE_MARGIN` score-margin gate is removed. The debug log emits `sem_sim`/`score`/`exact`/`kw`/`cap`/`verdict`/`inline` per search.
- **Single-snapshot version consistency now covers the doc body's own version bullet** (`_reconcile_body_version`): rc2 synced only the pin banner, but the curated body's `**Latest Version**` bullet could still be stale and contradict both the banner and `results[].latest_version` in the same response (3 of 55 modules in Run 9 traffic). `orientation_head` rewrites the body bullet to the threaded snapshot at render time (emitted text only — embeddings untouched, the only drift-safe fix).
- **Single-snapshot version consistency**: `search_modules(expand_top=True)`'s inlined `top_module_doc` previously re-parsed the version pin from the doc body, which could drift from the separately stored `results[].latest_version` metadata field after a metadata-only patch and propagate a wrong pin to a final answer. `orientation_head`/`_version_pin_hint` now accept a `version_override` and `search_modules_impl` threads `results[].latest_version` into it, so the two can never contradict each other in one response.
- **The `get_module` footer disclaimer is a compact 1-2 line pointer**, down from a 764-char paragraph repeated verbatim on every call (56/56 in the addendum measurement, 42.8K chars of pure repetition). The escalation stays load-bearing: curated subset, the COMPLETE inputs/outputs with exact types/defaults and nested shapes plus a name-exists check via `grep_module_docs` on the Module ID above, and resource-creation conditions live in module source. The Available sections menu and omitted/unmatched lines are unchanged.
...
- **The default `get_module` orientation head caps its inlined input table to a bounded sample** — the leading rows (the terraform-aws-modules docs front-load the inputs that matter) plus any row the doc explicitly marks required that falls outside that window, appending a `(+N more inputs → get_module(sections=["inputs"]))` pointer. Claws back the +31% head growth the 0.20.0 double-fetch fix introduced — measured 11.6–19.8% smaller heads on s3-bucket/vpc/elasticache/apigateway-v2/opensearch. The full table stays one `sections=["inputs"]` call away; the head table is never empty.
...
- **Scoped `grep_module_docs` matches no longer lose their container name.** A match on a continuation line of a multi-line row previously reported only the raw line text; two of three condition-A workers in the Run 8 transcripts back-filled a plausible-but-wrong variable name from the surrounding prose and emitted invalid HCL as a result. See the `enclosing` field above.
- **`search_modules(expand_top=True)` could inline a version pin that contradicted `results[].latest_version`** in the same payload (observed on global-accelerator: `3.0.1` in the head vs `3.0.0` in the result row). See single-snapshot version consistency above.
- **The inlined head's body `**Latest Version**` bullet could contradict its own reconciled pin banner** — rc2 fixed the banner but not the body bullet the banner sits above; Run 9 saw the two disagree in 3 of 55 modules' traffic. Now reconciled to one snapshot at render time (see single-snapshot version consistency above).
- **Non-top-1 result `description` blurbs truncated mid-word** — the char-cap clip left a dangling partial word in every rank ≥ 2 row. `_clip_blurb` now clips on the last word boundary, matching `extract_description`.
- **A module doc leaked raw YAML front-matter into its renders** — only `wafv2.md` opened with a `---`/`module_name:`/`keywords:` block, which `extract_description` skipped the fences of but kept the body lines from (the search-result `description`), and which rendered verbatim at the top of its `get_module` and search-inline output (the doc body). The block was redundant — its `keywords:` duplicate the `## Module Information` Keywords bullet the parser actually reads — so it was deleted from the page (re-encoded wafv2; golden set held 172/172). `_strip_yaml_frontmatter` in `extract_description` stays as defense, and a doc-schema lint guard (`test_no_doc_opens_with_yaml_frontmatter`) fails any curated page that opens with a front-matter fence.
- **The S3 `server_side_encryption_configuration` input (an `any`) now names its canonical nested keys** (`rule` → `apply_server_side_encryption_by_default` → { `sse_algorithm`, `kms_master_key_id` }, `bucket_key_enabled`) so a grep of the curated doc finds them. Re-encoded s3-bucket only; the row sits past the embedding-model truncation window, so its vector is byte-identical (zero embedding drift).

### Unchanged

- **The search index is not rebuilt; only two docs are re-encoded, drift-safe** — s3-bucket (SSE-shape edit; the edit lands past the embedding-model truncation window, so its vector row stays byte-identical) and wafv2 (front-matter removed from the first tokens, so its vector row does change). Every other doc's embedding is byte-identical to 0.21.0 and the golden set holds 172/172. `compute_scores` is a thin wrapper over the new `compute_scores_detailed`, so every existing caller (CLI, tests) is unaffected.
- **No network, no new dependency, no tool-schema regression.** All changes are in `tfmod_search_lib.py`/`tfmod_mcp_server.py`/`tfmod_doc_grep.py` response shaping plus tests; `get_module`'s full-document and `sections=["all"]` paths, core sections, transports, ONNX/torch backends, shared-HTTP-daemon and proxy modes, and the Docker build are all unaffected.
