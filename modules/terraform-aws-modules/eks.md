...
17. **Two Cluster Security-Group Outputs Are NOT Interchangeable**: `cluster_primary_security_group_id` is the SG that Amazon EKS creates and manages for the control plane — it is the value behind a raw `aws_eks_cluster.<name>.vpc_config[0].cluster_security_group_id` reference. `cluster_security_group_id` is a SEPARATE, additional SG that this module creates only when `create_security_group = true` (empty otherwise). When replacing a raw `vpc_config[0].cluster_security_group_id` output, wire it to `cluster_primary_security_group_id`, NOT the name-similar `cluster_security_group_id`.
