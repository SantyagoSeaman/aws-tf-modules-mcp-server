...
| `create_topic_policy` | `bool` | `true` | Whether to create and attach an `aws_sns_topic_policy` |
...
| `topic_policy` | `string` | `null` | Externally authored JSON policy, used verbatim when `create_topic_policy = false` |
...
5. **`topic_policy` vs `topic_policy_statements`**: `topic_policy` (a raw JSON string) is only applied when `create_topic_policy = false`; when `create_topic_policy = true` (the default), the module builds the policy from `enable_default_topic_policy` + `topic_policy_statements` + `source_topic_policy_documents`/`override_topic_policy_documents` instead, and `topic_policy` is ignored.
