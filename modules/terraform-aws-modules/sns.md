...
| `topic_policy_statements` | `map(object)` | `null` | Custom IAM statements (actions, principals, conditions) merged into the generated policy — fields: `sid`, `actions`, `not_actions`, `effect`, `resources`, `not_resources`, `principals`, `not_principals`, … (8 shown; call get_module with sections=["inputs","outputs"] for the complete list) |
