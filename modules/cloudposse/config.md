...
10. **Verify Managed Rule Identifiers**: Confirm exact AWS Config Managed Rule `identifier` values via the AWS documentation before pinning them in `managed_rules` — identifiers are case-sensitive AWS-defined constants (e.g. `S3_BUCKET_PUBLIC_READ_PROHIBITED`).
