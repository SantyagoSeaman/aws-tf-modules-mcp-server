...
5. **`event_selector` vs `advanced_event_selector`**: Configure at most one of these two data-event mechanisms per trail; prefer `advanced_event_selector` for new configurations and verify the exact field shape via `get_module(sections=["inputs","outputs"])` before writing entries, since both are nested `list(object({...}))` types easy to get wrong from memory.
...
8. **Verify Exact Variable Shapes via `get_module`**: Cloud Posse modules occasionally rename or add label inputs across minor versions — confirm `event_selector`/`advanced_event_selector`/`insight_selector` object fields with `get_module(sections=["inputs","outputs"])` against the pinned `0.24.0` docs before relying on memorized shapes.
