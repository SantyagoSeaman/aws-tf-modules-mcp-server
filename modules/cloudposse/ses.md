...
6. **Verify Exact Variable Names via `grep_module_docs`**: Cloud Posse modules rename/add label inputs across minor versions occasionally — confirm `iam_permissions`/`custom_from_*`/`zone_id` names and defaults against the pinned `0.25.2` docs before relying on memorized values.
