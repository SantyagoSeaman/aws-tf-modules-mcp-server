...
7. **Verify Exact Variable Names via `grep_module_docs`**: Cloud Posse modules occasionally rename or add label inputs across versions — confirm `subscribers`/`runtime_monitoring_additional_config`/detection-toggle names and defaults against the pinned `1.0.0` docs before relying on memorized values.
