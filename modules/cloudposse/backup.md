...
9. **Pre-1.x Examples May Use Removed Syntax**: The backup-schedule declaration syntax changed in `0.14.0`, and the deprecated pre-0.14.0 variables were removed entirely as of `1.x` — verify any copied example against the pinned `1.1.1` docs (or `grep_module_docs`) rather than an older blog post or Stack Overflow answer.
